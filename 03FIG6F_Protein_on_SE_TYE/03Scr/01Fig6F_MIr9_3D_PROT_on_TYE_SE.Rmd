...
library(openxlsx)
library(readxl)
MAT_A = read_excel("../02formR/05d100pe3MLg_SE_TYE_M3_d200_001_A2/00_05d100pe3MLg_SE_TYE_M3_d200_001_A2_MAT_fPLOTs_SE.xlsx")
```
...
## MAKE ORIGINAL plort calculated  from NGSPLOT Without Smoothng the line
...
P11t<- ggplot(MAT_A, aes(x=TSS, y=RPMM2, group= as.factor(M9),colour=as.factor(M9))) +geom_line(size=0.5)+  facet_wrap(~HM+DE, ncol =2,strip.position="top",scales = "free")  + scale_color_manual(name="DE",values=CO1) +theme_classic()+  theme(axis.text.y   = element_text(size=LE,angle = 0, face="bold"),legend.position = "rigt",panel.border = element_rect(colour = "black", fill=NA, size=BOR),axis.text.x=element_text(angle = 0,size=SI , face="bold"),axis.title.x  =element_text(angle = 0,size=SI , face="bold"),axis.title.y = element_text(angle = 90,size=SI , face="bold"),strip.text.y.left = element_text(angle = 0,size=10 , face="bold")) +  scale_x_continuous(breaks = c(1,200, 401), labels = c(A1,A2,A4))+ labs(title=paste(PROJECT,"_Fixed_Y_scale",sep="" ) , x=XL, y = YL) 
print(P11t)
P11t2<- ggplot(MAT_A, aes(x=TSS, y=RPMM2, group= as.factor(M9),colour=as.factor(M9))) +geom_line(size=0.5)+  facet_wrap(~HM+DE, ncol =2,strip.position="top",scales = "fixed") +theme_classic()+ scale_color_manual(name=CELL,values=CO1)+theme(axis.text.y   = element_text(size=LE,angle = 0, face="bold"),legend.position = "rigt",panel.border = element_rect(colour = "black", fill=NA, size=BOR),axis.text.x=element_text(angle = 0,size=SI , face="bold"),axis.title.x  =element_text(angle = 0,size=SI , face="bold"),axis.title.y = element_text(angle = 90,size=SI , face="bold"),strip.text.y.left = element_text(angle = 0,size=10 , face="bold")) +  scale_x_continuous(breaks =c(1,200, 401), labels = c(A1,A2,A4))+ labs(title=paste(PROJECT,"_Fixed_Y_scale",sep="" ) , x=XL, y = YL) 
print(P11t2)
...
pdf(file=paste(PORT,PROJECT,"_ORI_f_NGSPLOT",".pdf",sep=""),width=4 , height=35)  
print(arrange1t) 
print(arrange12t)            
dev.off()
...
SAMPLES_to_PLOtS<- data.frame(c("H3K27ac_Ctr", "EP300_endo", "SMARCA1_WT","Rad21_r12","SMARCA4_WT",    
                                "Med1_SRR1519931","Klf4_endo","Smad3_SRR064924","Hdac1_SRR5077718", "Hdac2_SRR675","SMARCA5_wt" ,"CHD4_wt"))

SAMPLES_to_PLOtS2<- c("H3K27ac_Ctr", "EP300_endo", "SMARCA1_WT","Rad21_r12","SMARCA4_WT",     "Med1_SRR1519931","Klf4_endo","Smad3_SRR064924","Hdac1_SRR5077718", "Hdac2_SRR675","SMARCA5_wt","CHD4_wt")

colnames(SAMPLES_to_PLOtS)<- "Sample"

TTEMP<- merge(SAMPLES_to_PLOtS,MAT_A,by="Sample")
table(TTEMP$Sample)
mat_RPKM1_5_c <- subset(TTEMP,  TSS>=170 & TSS <= 230)
...
#c1<-  ggplot(stat.test6, aes(factor(COMBI), log10p, fill = factor(COMBI))) +  geom_bar(position = "stack",stat = "identity") + guides(fill = guide_legend(reverse = F)) +labs(fill = "clu") + theme_bw() + scale_fill_manual(values =COeP)  +  theme(plot.title= element_text(color=BASIC_COL, size=LE, face="bold"), axis.text.y= element_text(size=LE,face="bold"),axis.text.x = element_text(size=SI, face="bold",angle=-90,h=0), axis.title.x =element_text(color=BASIC_COL, size=LE, face="bold"), axis.title.y = element_text(color=BASIC_COL, size=LE, face="bold"), panel.border = element_rect(colour = BASIC_COL, fill=NA, size=1))+ labs(title=paste(PROJECT,TRY,sep="_" ) , x=XL2, y = YL2)+coord_flip()
#print(c1)
#YL3<- "log10 adjuested Pvalue t_test"

 # stat.test6$COMBI<- factor(stat.test6$COMBI, levels =unique(stat.test6$COMBI[order(stat.test6$log10fdr,decreasing=F)]))
#c11<-  ggplot(stat.test6, aes(factor(COMBI), log10fdr, fill = factor(COMBI))) +  geom_bar(position = "stack",stat = "identity") + guides(fill = guide_legend(reverse = F)) +labs(fill = "clu") + theme_bw() + scale_fill_manual(values =COeP)  +  theme(plot.title= element_text(color=BASIC_COL, size=LE, face="bold"), axis.text.y= element_text(size=LE,face="bold"),axis.text.x = element_text(size=SI, face="bold",angle=-90,h=0), axis.title.x =element_text(color=BASIC_COL, size=LE, face="bold"), axis.title.y = element_text(color=BASIC_COL, size=LE, face="bold"), panel.border = element_rect(colour = BASIC_COL, fill=NA, size=1))+ labs(title=paste(PROJECT,TRY,sep="_" ) , x=XL2, y = YL3)+coord_flip()
#print(c11)
#arrangeBAR1 <- ggarrange(c1, ncol = 1,nrow =1,common.legend = T, align = c("hv"),legend="none")
#arrangeBAR2 <- ggarrange(c11, ncol = 1,nrow =1,common.legend = T, align = c("hv"),legend="none")
   
#pdf(file=paste(PORT,"003_",PROJECT, "_PATH_BAR_PVALUE",".pdf",sep=""), width=5, height=4)  
#print(arrangeBAR1)
#print(arrangeBAR2)
#dev.off()
...
## Prepare matrix to make the Dotplot
```{r,message=FALSE,class.source="bg-info",warning=FALSE,echo=T}
...
#df5$Sample = factor(df5$Sample, levels=rev(SAMPLES_to_PLOtS2))
HAT= "Enrich of H3k27ac Prot +/- M9"

  Hesc_TMM_filter<- Heatmap(df5[,NAME_zD3] , name="zcore", cluster_columns = T,cluster_rows = T, col=colorRamp2(c(L, 0, M), CC), show_row_names = T,  row_names_gp = gpar(fontsize = SI),  show_column_names = T,column_order = IMPORT,rect_gp = gpar(col = "white", lwd = 2),
    column_title = HAT)
 
  draw(Hesc_TMM_filter, heatmap_legend_side = "left", annotation_legend_side = "left", merge_legend = TRUE)

  Hesc_TMM_filter2<- Heatmap(df5[,NAME_zD3] , name="zcore", cluster_columns =F,cluster_rows = F, col=colorRamp2(c(L, 0, M), CC),left_annotation = h, show_row_names = T,  row_names_gp = gpar(fontsize = SI),  show_column_names = T,column_order = IMPORT,rect_gp = gpar(col = "white", lwd = 2),
    column_title = HAT)
   draw(Hesc_TMM_filter2, heatmap_legend_side = "left", annotation_legend_side = "left",
       merge_legend = TRUE)
...
draw(Hesc_TMM_filter3, heatmap_legend_side = "left", annotation_legend_side = "left",
       merge_legend = TRUE)
 Hesc_TMM_filter4<- Heatmap(df5[,NAME_zD3] , name="zcore", cluster_columns =T,cluster_rows = F, col=colorRamp2(c(L, 0, M), CC),left_annotation = h, show_row_names = T,  row_names_gp = gpar(fontsize = SI),  show_column_names = T,column_order = IMPORT,rect_gp = gpar(col = "white", lwd = 2),
    column_title = HAT)
   draw(Hesc_TMM_filter2, heatmap_legend_side = "left", annotation_legend_side = "left",
       merge_legend = TRUE)
{ pdf(file=paste(PORT,PROJECT,"HEATP_2_11.pdf",sep=""), width=3.4, height=4) 
  set.seed(1)
   print(Hesc_TMM_filter)
    draw(Hesc_TMM_filter2, heatmap_legend_side = "left", annotation_legend_side = "left",
       merge_legend = TRUE)
     draw(Hesc_TMM_filter3, heatmap_legend_side = "left", annotation_legend_side = "left",
       merge_legend = TRUE)
     print(Hesc_TMM_filter4)
}
dev.off()
```
