...
# **Objective**
-To generate Hockey stick plot after analysis using the ROSE algorithm and showing distribution of normalized H3K27ac CUT&Tag signal across typical enhancers (TYE) and super-enhancers (SE) in Ctrl (gray line) or miR-9 antagomir(green line) transfectedMLg cells<br /><br />
...
## Prepare the matrix to perform the plot
```{r,class.source="bg-info",echo=TRUE,warning=FALSE}
df.list<- list()
...
# print(F1)
F2<- ggplot(GN5a) + geom_line(aes(XLA,(Normalized_Tag_Count), colour=ID),size=2) +scale_color_manual(name=GR1,values=CO22S)+ theme_bw() + theme(strip.placement = "outside",strip.text.y.left = element_text(angle=0),axis.text.x=element_text(angle = 0,size=LL, face="bold"),panel.border = element_rect(colour = BASIC_COL,  size=Si),axis.text.y = element_text(size=LL, face="bold")) + labs(title=paste(PROJECT,sep="" ) , x=XL, y = YL2)+scale_x_continuous(breaks = c( L1,L3,L4 ))+facet_zoom(ylim = c(0, S3), zoom.data = ifelse(a <= S2, NA, FALSE))
  
F3<- ggplot(GN5a) + geom_line(aes(XLA,Normalized_Tag_Count, colour=ID),size=2) +scale_color_manual(name=GR1,values=CO22S)+   theme_bw() +    theme(strip.placement = "outside",strip.text.y.left = element_text(angle=0),axis.text.x=element_text(angle = 0,size=LL, face="bold"),panel.border = element_rect(colour = BASIC_COL,  size=Si),axis.text.y = element_text(size=LL, face="bold")) + labs(title=paste(PROJECT,sep="" ) , x=XL, y = YL2)+scale_x_continuous(breaks = seq(L1, L4, by = 3999)) +scale_y_continuous(limits = c(0, S4), breaks = seq(0, S4, by = S4/2))
...
dev.off()
```
##Ccheck for MLg cells Peak Size of H3K27ac in Ctr and Mir9-LOF
...
## Barplot from SE and TYE by size
...
## Create the Matrix of Percentage for MLg to calculate the P-value
